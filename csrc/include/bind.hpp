...
void init_refdata(py::module& m) {
    py::class_<RefData, std::shared_ptr<RefData>>(m, "RefData")
        .def(py::init<>())
        .def("__str__", [](std::shared_ptr<RefData> self) { return self->__str__(); })
        .def("__setitem__", [](std::shared_ptr<RefData> self, std::string key, std::string value) { self->__setitem__(key, value); })
        .def("__getitem__", [](std::shared_ptr<RefData> self, std::string key) { return self->__getitem__(key); })
        .def("__delitem__", [](std::shared_ptr<RefData> self, std::string key) { self->__delitem__(key); })
        .def("__iter__", [](std::shared_ptr<RefData> self) -> std::shared_ptr<RefData> {
            self->__iter__();
            return self;
        })
        .def("__next__", [](std::shared_ptr<RefData> self) { return self->__next__(); })
        .def("__repr__", [](std::shared_ptr<RefData> self) { return self->__str__(); })
        .def("__contains__", [](std::shared_ptr<RefData> self, std::string key) { return self->__contains__(key); })
        .def("keys", [](std::shared_ptr<RefData> self) { return self->keys(); })
        .def("values", [](std::shared_ptr<RefData> self) { return self->values(); });
}
...
json pyobject_to_json_via_string(py::object obj) {
    py::object json_dumps = py::module_::import("json").attr("dumps");
    std::string json_string = json_dumps(obj).cast<std::string>();
...
void init_component(py::module& m) {
    py::class_<ComponentBase, PyComponentBase, std::shared_ptr<ComponentBase>>(m, "ComponentBase")
        ...
        .def("__or__", [](std::shared_ptr<PyComponentBase> self, py::object obj) {
            json style_obj = pyobject_to_json_via_string(obj);
            return self | utils::parse_element_style<json, Element>(style_obj);
        })
        ;
}