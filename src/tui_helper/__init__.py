...
def layout(type, config: list) -> ComponentBase:
    ...
    if type == "split":
        ...
        is_deepsplit_h = lambda x: is_list(x) and len(x) == 3 and all([f(c) for c, f in zip(x, [is_item, is_hseparator, is_item])])
        is_deepsplit_v = lambda x: is_list(x) and len(x) == 3 and all([f(c) for c, f in zip(x, [is_item, is_vseparator, is_item])])
        ...
        while len(dq) > 1:
            if len(dq) < 3 or (not is_hseparator(dq[1]) and not is_vseparator(dq[1])):
                raise ValueError(f"Invalid layout configuration near: '{dq[1]}'")
            c = [dq.popleft() for _ in range(3)]
            # block4
            if is_deepsplit_v(c[0]) and is_hseparator(c[1]) and is_deepsplit_v(c[2]):
                if is_split_v(c[0]) and is_split_v(c[2]):
                    dq.appendleft(layout_resizable_split(SplitLayoutType.Block4, cc(c[0] + c[2])))
                else:
                    # handle is_split_v level return [split_v, '---',  split_v]
                    dq.extendleft(reversed([ci if not is_deepsplit_v(ci) else [d if not is_list(d) else layout(type, d) for d in ci] for ci in c]))
            elif is_deepsplit_h(c[0]) and is_vseparator(c[1]) and is_deepsplit_h(c[2]):
                if is_split_h(c[0]) and is_split_h(c[2]):
                    dq.appendleft(layout_resizable_split(SplitLayoutType.Block4, [c[0][0], c[2][0], c[0][2], c[2][2]]))
                else:
                    # handle is_split_h level return [split_h, '|',  split_h]
                    dq.extendleft(reversed([ci if not is_deepsplit_h(ci) else [d if not is_list(d) else layout(type, d) for d in ci] for ci in c]))
            # block3up
            ...
            # block2vertical
                ...
                # handle component level return [component, '---' or '|',  component]
                dq.extendleft(reversed([ci if not is_list(ci) else layout(type, ci) for ci in c]))
        return dq[0]
    else:
